...
if (!token) {
  ...
  loadPlayerCatsAndJoin(token);
}

async function loadPlayerCatsAndJoin(token) {
  try {
    ...
    if (!Array.isArray(cats) || cats.length === 0) {
      console.error('❌ No cats found for this user.');
      return;
    }

    const playerId = cats[0].player_id;
    const catId = cats[0].cat_id;

    console.log(`🐾 Loaded player ID: ${playerId}`);
    console.log(`🐱 Using cat ID: ${catId}`);

    // Now join the fashion show via socket
    const socket = io('https://catwalk.onrender.com');

    socket.on('connect', () => {
      console.log('🔌 Connected to fashion show socket:', socket.id);
    ...
    console.error('❌ Failed to fetch cats or join game:', err);
  }
}
